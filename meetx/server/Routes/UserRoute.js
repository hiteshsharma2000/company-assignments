  ...
  try {
     ...
     if(getuser){
        bcrypt.compare(password,getuser.password,function(err,result){
            if(result){
                var token = jwt.sign({id:getuser._id,name:getuser.name,email:getuser.email,dob:getuser.dob}, "user",{expiresIn:"1h"});
                res.send({"msg":"login successfully","token":token,"user":getuser})
            }else{
